...
if __name__ == "__main__":
    ...
    credit_file_loo = "/data/projects/monet/atlas/experiments/large_t5_model_lm_TRUE_leave_one_out/nq_test-step-0_input_reduction.jsonl"
    credit_file_attention = "/data/projects/monet/atlas/experiments/large_t5_model_lm_TRUE_attention_credit/nq_test-step-0-step-0-eval.jsonl"
    credit_file_lca_multi_doc_soft = "/data/projects/monet/atlas/experiments/large_t5_model_lm_TRUE_lca_multiple_document_generation_softmax_20231214-111236/nq_test-step-0-step-0-eval-step-1000.jsonl"
    credit_file_cosine = "/data/projects/monet/atlas/experiments/large_t5_model_lm_TRUE_cosine_credit_20231214-154254/nq_test-step-0-step-0-eval-step-0-eval.jsonl"
    ...
    for i in range(len(data_lca_multi_doc_soft)):
        ...
        learned_performance[data_lca_multi_doc_soft[i]['query']] = data_lca_multi_doc_soft[i]['f1_pred']        
    
    cosine_dict = {}
    cosine_scores = {}
    for i in range(len(data_cosine)):
        try:
            temp = len(data_cosine[i]['cosine_credit']) - ss.rankdata(np.array(data_cosine[i]['cosine_credit']) * learned_performance[data_cosine[i]['query']], method='max').astype(int) + 1
        except KeyError:
            continue
        cosine_dict[data_cosine[i]['query']] = temp
        cosine_scores[data_cosine[i]['query']] = np.array(data_cosine[i]['cosine_credit']) * learned_performance[data_cosine[i]['query']]
    
    attention_dict = {}
    attention_scores = {}
    for i in range(len(data_attention)):
        try:
            temp = len(data_attention[i]['attention_credit']) - ss.rankdata(np.array(data_attention[i]['attention_credit']) * learned_performance[data_attention[i]['query']], method='max').astype(int) + 1
        except KeyError:
            continue
        attention_dict[data_attention[i]['query']] = temp
        attention_scores[data_attention[i]['query']] = np.array(data_attention[i]['attention_credit']) * learned_performance[data_attention[i]['query']]
