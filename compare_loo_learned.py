...
if __name__ == "__main__":
    ...
    for i in range(len(data_gradient)):
        try:
            temp = len(data_gradient[i]['gradient_credit']) - ss.rankdata(np.array(data_gradient[i]['gradient_credit']) * learned_performance[data_gradient[i]['query']], method='max').astype(int) + 1
        except KeyError:
            continue 
        gradient_dict[data_gradient[i]['query']] = temp
        gradient_scores[data_gradient[i]['query']] = np.array(data_gradient[i]['gradient_credit']) * learned_performance[data_gradient[i]['query']]
