...
size=base
RETRIEVE_DIR=/data/projects/monet/atlas/experiments/base_t5_model_lm_TRUE_credit_lca_multiple_document_generation_${size}
...
EVAL_FILES="${EXPERIMENT_NAME}/trivia_train-step-0.jsonl ${EXPERIMENT_NAME}/triviaqa_dev-step-0.jsonl ${EXPERIMENT_NAME}/triviaqa_test-step-0.jsonl ${EXPERIMENT_NAME}/nq_dev-step-0.jsonl ${EXPERIMENT_NAME}/train-step-0.jsonl ${EXPERIMENT_NAME}/nq_test-step-0.jsonl"
...
# combine json files
python combine_json_files.py --input_dir ${EXPERIMENT_NAME}

# Train on the generated data
TRAIN_FILES="${EXPERIMENT_NAME}/train_nq_trivia.jsonl"
EVAL_FILES="${EXPERIMENT_NAME}/dev_nq_trivia.jsonl ${EXPERIMENT_NAME}/test_nq_trivia.jsonl"

CUDA_LAUNCH_BLOCKING=1 python -m torch.distributed.launch --nproc_per_node 8 learned_credit.py --name ${EXPERIMENT_NAME} --generation_max_length 32 --target_maxlength 32 --gold_score_mode "ppmean" --precision ${PRECISION} --reader_model_type google/t5-${size}-lm-adapt --text_maxlength 512 --target_maxlength 16 --model_path ${PRETRAINED_MODEL} --load_index_path ${PRETRAINED_INDEX} --train_data ${TRAIN_FILES} --eval_data ${EVAL_FILES} --per_gpu_batch_size 64 --n_context 5 --retriever_n_context 40 --checkpoint_dir ${SAVE_DIR} --index_mode "flat" --task "qa_retrieved" --qa_prompt_format "{question}" --write_results --local_rank 0 --total_steps 1000 --save_freq 50 --lca_multiple_document_generation --per_gpu_batch_size_eval 32
