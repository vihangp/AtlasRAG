...
if __name__ == "__main__":
    ...
    for key in lca_dict.keys():
        ...
        lca_kl = 0
        lca_kl_soft = 0
        for i,p in enumerate(loo_ranks):
            grad_q = gradient_ranks[i]
            att_q = attention_ranks[i]
            lca_q = lca_ranks[i]
            lca_q_soft = lca_ranks_soft[i]

            grad_kl += p * np.log((p + 1e-8) / grad_q)
            att_kl += p * np.log((p + 1e-8) / att_q)
            lca_kl += p * np.log((p + 1e-8) / lca_q)
            lca_kl_soft += p * np.log((p + 1e-8) / lca_q_soft)


        grad_mrr.append(grad_kl)
        att_mrr.append(att_kl)
        lca_mrr.append(lca_kl)
        lca_mrr_soft.append(lca_kl_soft)
    
    print("Gradient KL:", np.mean(grad_mrr))
    print("Attention KL:", np.mean(att_mrr))
    print("LCA KL:", np.mean(lca_mrr))
    print("LCA KL Softmax:", np.mean(lca_mrr_soft))
